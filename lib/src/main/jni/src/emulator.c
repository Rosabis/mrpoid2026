...
Java_com_mrpoid_core_Emulator_native_1init(JNIEnv *env, jobject self, jobject mrpScreen, jobject emuAudio)
{
#if 0
...
static void *g_extData = NULL;
static int32_t g_extLen = 0;

JNIEXPORT void JNICALL
Java_com_mrpoid_core_Emulator_native_1loadExtData(JNIEnv *env, jobject self, jbyteArray data)
{
    if (g_extData) {
        free(g_extData);
        g_extData = NULL;
    }
    g_extLen = (*env)->GetArrayLength(env, data);
    g_extData = malloc(g_extLen);
    (*env)->GetByteArrayRegion(env, data, 0, g_extLen, (jbyte *)g_extData);
    LOGI("loadExtData: %d bytes loaded", g_extLen);
}
...
Java_com_mrpoid_core_Emulator_native_1startMrp(JNIEnv * env, jobject self, jstring path)
{
	...
	if(str)
	{
		...
		strncpy(runMrpPath, str, sizeof(runMrpPath) - 1);
		runMrpPath[sizeof(runMrpPath) - 1] = '\0';
        ...
        if (!g_extData || g_extLen <= 0) {
            LOGE("cfunction.ext not loaded! Call native_loadExtData first.");
            return -1;
        }

        LOGI("uc_startVmrp calling with mrpPath='%s' scn=%dx%d", runMrpPath, SCNW, SCNH);
        int ret = uc_startVmrp(g_extData, g_extLen, runMrpPath, SCNW, SCNH);
        LOGI("uc_startVmrp returned %d", ret);
		return ret == MR_SUCCESS ? 1 : -1;
	}
...
JNIEXPORT void JNICALL
Java_com_mrpoid_core_Emulator_native_1pause(JNIEnv *env, jobject self)
{
	LOGD("native pause!");
	m_bSuspendDraw = 1;
...
JNIEXPORT void JNICALL
Java_com_mrpoid_core_Emulator_native_1resume(JNIEnv *env, jobject self)
{
	LOGD("native resume!");
	m_bSuspendDraw = 0;
...
JNIEXPORT void JNICALL
Java_com_mrpoid_core_Emulator_native_1stop(JNIEnv *env, jobject self)
{
	LOGD("native stop!");
...
JNIEXPORT void JNICALL
Java_com_mrpoid_core_Emulator_native_1event(JNIEnv *env, jobject self, jint code, jint p0, jint p1)
{
	if(gEmuEnv.showFW) LOGI("mr_event(%d, %d, %d)", code, p0, p1);

	if(code == MR_EMU_ON_TIMER) {
		uc_timer();
		return;
	}
...
JNIEXPORT jint JNICALL
Java_com_mrpoid_core_Emulator_native_1smsRecv(JNIEnv *env, jobject self, jstring content, jstring num)
{
...
JNIEXPORT void JNICALL
Java_com_mrpoid_core_Emulator_native_1destroy(JNIEnv * env, jobject self)
{
 ...
 */
JNIEXPORT void JNICALL
Java_com_mrpoid_core_Emulator_native_1callback(JNIEnv * env, jobject self, int what, int param)
{
	if(!gEmuEnv.b_vm_running) {
...
JNIEXPORT void JNICALL
Java_com_mrpoid_core_Emulator_native_1screenReset(JNIEnv * env, jobject self, jobject cacheBitmap, jobject realBitmap, jint width, jint height)
{
...
Java_com_mrpoid_core_Emulator_native_1getStringOptions(JNIEnv * env, jobject self, jstring key)
{
	const char *str, *str2;
	jstring ret = NULL;
...
JNIEXPORT void JNICALL
Java_com_mrpoid_core_Emulator_native_1setStringOptions(JNIEnv * env, jobject self, jstring key, jstring value)
{
	const char *str, *str2;
	char buf[128];
...
JNIEXPORT void JNICALL
Java_com_mrpoid_core_Emulator_native_1setIntOptions(JNIEnv * env, jobject self, jstring key, jint value)
{
	const char *str;
...
JNIEXPORT void JNICALL
Java_com_mrpoid_core_Emulator_native_1getMemoryInfo(JNIEnv * env, jobject self)
{
	uint32 len, left, top;
...
JNIEXPORT jint JNICALL
Java_com_mrpoid_core_Emulator_native_1handleMessage(JNIEnv * env, jobject self, jint what, jint p0, jint p1)
{
...
JNIEXPORT jstring JNICALL
Java_com_mrpoid_core_Emulator_native_1getAppName(JNIEnv * env, jobject self, jstring path)
{
	const char *str;
...
JNIEXPORT void JNICALL
Java_com_mrpoid_core_Emulator_native_1lockBitmap(JNIEnv * env, jobject self)
{
	(*env)->MonitorEnter(env, obj_realBitmap);
}

JNIEXPORT void JNICALL
Java_com_mrpoid_core_Emulator_native_1unLockBitmap(JNIEnv * env, jobject self)
{

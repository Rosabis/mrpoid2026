...
/*
 * Use portable fixed-width types from mr_stdint.h
 * This ensures consistent type sizes across 32-bit and 64-bit platforms.
 */
#include "include/mr_stdint.h"

/* Legacy type definitions - now handled by mr_stdint.h
 * The following #if block is kept for reference but disabled.
 */
#if 0
#define MR_C_NUMBER_TYPE
typedef  unsigned short     uint16;      /* Unsigned 16 bit value */
typedef  unsigned long int  uint32;      /* Unsigned 32 bit value - BROKEN on 64-bit! */
typedef  long int  int32;      /* signed 32 bit value - BROKEN on 64-bit! */

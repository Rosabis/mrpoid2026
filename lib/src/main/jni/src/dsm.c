...
void mr_getMemoryInfo(uint32 *total, uint32 *free_mem, uint32 *top)
{
	if (total) *total = 0;
...
{
	uint64_t disk_free_space, disk_total_space;
	int32 fs_ret;
...
//		LOGI("dsmGetFreeSpace() ret=%d", fs_ret);
	...
	if(fs_ret == 0)
	{
		//disk_free_space	= (uint64_t)1024*1024*1024*2;
		//disk_total_space  = (uint64_t)1024*1024*1024;
		disk_total_space = disk_statfs.f_blocks * disk_statfs.f_bsize;
		disk_free_space = disk_statfs.f_bfree * disk_statfs.f_bsize;

		if((disk_total_space/10) >(uint64_t)1024*1024*1024)
		{
		...
		if((disk_free_space/10) >(uint64_t)1024*1024*1024)
		{

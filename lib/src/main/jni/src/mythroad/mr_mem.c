...
int32 mr_mem_init(void) {
    ...
    /* Fixed: Use uintptr_t for pointer arithmetic to work on 64-bit platforms */
    LG_mem_base = (char*)(((uintptr_t)Origin_LG_mem_base + 3) & ~(uintptr_t)3);
    LG_mem_len = (Origin_LG_mem_base + Origin_LG_mem_len - LG_mem_base) & ~(int32)3;

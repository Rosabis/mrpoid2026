...
public class EmuScreen {
    ...
    /** Non-null while this screen is wired to a view; cleared when switching views. */
    private EmuView boundView;
    ...
    /**
     * Re-bind to a new {@link EmuView} after the activity was destroyed and recreated while the VM kept running.
     */
    public synchronized void attachToView(EmuView view) {
        if (view == null) {
            return;
        }
        if (boundView != null && surfaceHolder != null && surfaceCallback != null) {
            try {
        ...
        surfaceCallback = new SurfaceHolder.Callback() {
            ...
            public void surfaceCreated(SurfaceHolder h) {
                ...
                surfaceOk = true;
                surfaceHolder = h;
                // 从短信/权限等界面返回后 surface 会销毁再创建；若 MRP 未立刻重绘，flush 曾被跳过会导致黑屏
                postFlush();
            }

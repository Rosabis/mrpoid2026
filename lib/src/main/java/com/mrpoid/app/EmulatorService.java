...
public class EmulatorService extends AppProcessService {
	...
	private static final String CHANNEL_ID = "mrp_fg_channel";
	/** Same as {@code ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PLAYBACK} (API 34). */
	private static final int FGS_TYPE_MEDIA_PLAYBACK = 1 << 1;
	...
	private void createNotificationChannel() {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
			...
			channel.setDescription("MRP emulator is running");
	...
	private void handleStart(Intent intent) {
		...
		if (ACTION_STARTMRP.equals(intent.getAction())) {
			...
			PendingIntent appIntent = PendingIntent.getActivity(this, 0, notifyIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
			...
			n.flags = Notification.FLAG_NO_CLEAR; // 不可清除

			int nid = R.drawable.ic_notify + getProcIndex();
			if (Build.VERSION.SDK_INT >= 34) {
				startForeground(nid, n, FGS_TYPE_MEDIA_PLAYBACK);
			} else {

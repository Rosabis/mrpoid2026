...
推送或 PR 到 `main` / `master` / `develop` 时，工作流 [Android CI](.github/workflows/build-native.yml) 会在 Ubuntu 上安装 JDK 17、Android SDK、NDK 与 CMake，执行 `./gradlew :app:assembleDebug`，并上传 **Debug APK**（及可选的 native 产物）。也可在 Actions 里手动 **Run workflow**。打 `v*` 标签推送时会附带创建 Release 并附上构建产物。
